...
//		load('extend');//加载Thinkphp扩展函数库  why??
       ...
       //     $getConfig = getCache('cms.config.php', './');   //找不到该文件！！
            /*$getConfig=@require('./cms.config.php');
 ...
 //       Load('extend');//加载Thinkphp扩展函数库
    ...
    public function logout()
    {
        if(!empty($this->adminId)) {
            ...
            self::_message('success', '登出成功', U('Index/index'));
        }else {
            self::_message('success', '已经退出登录', U('Index/index'));
        }
    }
}
